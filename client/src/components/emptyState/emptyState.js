...
const DefaultEmptyState = ({ onAddCredential, onAddSource }) => {
  return (
    <Grid fluid>
      <Row>
        <EmptyState className="full-page-blank-slate">
          ...
          {onAddCredential &&
            onAddSource && (
              <React.Fragment>
                <EmptyState.Info>
                  A credential defines a set of user authentication information to be used during a scan.<br />A
                  credential includes a username and a password or SSH key. Entitlements Reporting uses SSH<br />to
                  connect to servers on the network and uses credentials to access those servers.
                </EmptyState.Info>
                <EmptyState.Action>
                  <DropdownButton
                    bsStyle="primary"
